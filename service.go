...
func NewTodoService(db *pgxpool.Pool, rdb *redis.Client) *TodoService {
	return &TodoService{db: db, rdb: rdb}
}
...
func (s *TodoService) CreateTodo(todo *Todo) (*Todo, error) {
	...
	log.Printf("📥 Inserting todo into DB: title=%q, completed=%v", todo.Title, todo.Completed)

	err := s.db.QueryRow(ctx,
		"INSERT INTO todos (title, completed, deleted, created_at, updated_at) VALUES ($1, $2, $3, $4, $5) RETURNING id",
	...
	if _, err := s.rdb.Del(context.Background(), "todos").Result(); err != nil {
		log.Printf("⚠️ Failed to invalidate Redis cache: %v", err)
	} else {
		log.Println("🧹 Redis cache invalidated for key 'todos'")
	}

	log.Printf("✅ Todo created with ID %d", todo.ID)
